...
if [ "$HELP" = true ] ; then
    ...
    echo "  --eval                  Install evaluation dependencies"
    return
fi
...
if [ "$BASIC" = true ] ; then
    pip install pillow imageio imageio-ffmpeg tqdm easydict opencv-python-headless scipy ninja rembg onnxruntime trimesh open3d xatlas pyvista pymeshfix igraph transformers clip image-reward torchmetrics
...
if [ "$EVAL" = true ] ; then
    pip install git+https://github.com/FeMa42/quality_assessment_library.git@1fcb3cdba4283fed901535945896412a1882310a
fi
