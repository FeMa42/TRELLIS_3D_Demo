...
class Trainer:
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        if self.is_master:
            ...
            # Initialize wandb
            self.wandb_run = None
            if self.wandb_config is not None and WANDB_AVAILABLE:
                try:
                    wandb_kwargs = dict(self.wandb_config)
                    if 'dir' not in wandb_kwargs:
                        wandb_kwargs['dir'] = self.output_dir
                    
                    # Extract and add full configuration for logging
                    if wandb_kwargs.get('log_full_config', True):
                        config_dict = self._prepare_wandb_config(
                            models=models,
                            dataset=dataset,
                            optimizer=optimizer,
                        ...
                        # remove log_full_config from wandb_kwargs to avoid logging it
                        wandb_kwargs.pop('log_full_config', None)
                        
                        # If there's already a config in wandb_kwargs, update it rather than replace
                        if 'config' in wandb_kwargs:
                            wandb_kwargs['config'].update(config_dict)
                        else:
                            wandb_kwargs['config'] = config_dict
                    ...
                    self.wandb_run = None
            elif self.wandb_config is not None and not WANDB_AVAILABLE:
                if self.is_master:
                    print('Warning: wandb config provided but wandb is not installed')
    ...
    def log_metrics(self, metrics_dict, step):
        """
        Log metrics to all configured backends (TensorBoard, wandb).
        """
        ...
        if self.wandb_run is not None:
            # Convert TensorBoard-style keys (with '/') to wandb format
            wandb_metrics = {}
            for key, value in metrics_dict.items():
                # Keep the hierarchical structure for wandb
    ...
    def prepare_dataloader(self, **kwargs):
        ...
        self.dataloader = DataLoader(
            ...
            num_workers= 10, # int(np.ceil(os.cpu_count() / torch.cuda.device_count())),
            pin_memory=True,
            drop_last=True,
    ...
    @abstractmethod
    def run_snapshot(self, num_samples, batch_size=4, verbose=False, dataset=None, **kwargs):
        """
    ...
    def snapshot(self, suffix=None, num_samples=64, batch_size=4, verbose=False):
        ...
        if self.eval_dataset is not None:
            current_sampling_dataset = self.eval_dataset
        else:
            current_sampling_dataset = self.dataset
        num_samples_per_process = int(np.ceil(num_samples / self.world_size))
        samples = self.run_snapshot(
                num_samples_per_process,
                batch_size=batch_size,
                verbose=verbose,
                dataset=current_sampling_dataset,
                )

        # evaluate sample
        if hasattr(current_sampling_dataset, 'eval_sample'):
            if 'sample_gt' in samples and 'sample' in samples:
                gt = samples['sample_gt']['value']
                pred = samples['sample']['value']
                try:
                    eval_result = current_sampling_dataset.eval_sample(gt, pred)
                    if eval_result is not None:
                        samples['eval'] = {'value': eval_result, 'type': 'metric'}
                        if isinstance(eval_result, dict):
                            for k, v in eval_result.items():
                                self.log_metrics({f'eval/{k}': v}, self.step)
                except Exception as e:
                    print(f"[Eval Dataset Eval] Error during evaluation: {e}")
        ...
        if self.is_master:
            ...
            for key in samples.keys():
                ...
                elif samples[key]['type'] == 'metric':
                    # safe metrics to json
                    with open(os.path.join(self.output_dir, 'samples', suffix, f'{key}_{suffix}.json'), 'w') as f:
                        # check if value is a dict
                        if isinstance(samples[key]['value'], dict):
                            json.dump(samples[key]['value'], f, indent=4)
                        elif isinstance(samples[key]['value'], (list, np.ndarray)):
                            json.dump(samples[key]['value'].tolist(), f, indent=4)
                        else:
                            json.dump(samples[key]['value'], f, indent=4)
        ...
        while self.step < self.max_steps:
            ...
            if self.is_master:
                ...
                if self.step % self.i_log == 0:
                    ...
                    self.log_metrics(log_show, self.step)
                    log = []
    ...
    def _prepare_wandb_config(self, **kwargs):
        """
        Prepare configuration for wandb logging.
        Extracts relevant information from models, dataset, and training parameters.
        """
        config = {}
        ...
        if 'models' in kwargs:
            ...
            for name, model in kwargs['models'].items():
                ...
                # Try to get model args if available
                if hasattr(model, 'args'):
                    model_config.update(model.args)
        ...
        if 'dataset' in kwargs:
            ...
            # Try to get dataset args if available
            if hasattr(dataset, 'args'):
                dataset_config.update(dataset.args)
            config['dataset'] = dataset_config
        
        # Extract training hyperparameters
        training_config = {}
        for key in [
            'batch_size', 'batch_size_per_gpu', 'batch_split', 'max_steps',
            'ema_rate', 'fp16_mode', 'fp16_scale_growth'
        ]:
            if key in kwargs:
                training_config[key] = kwargs[key]
        
        # Extract optimizer configuration
        if 'optimizer' in kwargs and kwargs['optimizer']:
            optimizer_config = kwargs['optimizer'].copy()
            if 'args' in optimizer_config and 'lr' in optimizer_config['args']:
                training_config['learning_rate'] = optimizer_config['args']['lr']
            config['optimizer'] = optimizer_config
        
        # Extract learning rate scheduler if available
        if 'lr_scheduler' in kwargs and kwargs['lr_scheduler']:
            config['lr_scheduler'] = kwargs['lr_scheduler']
        
        # Extract elastic controller if available
        if 'elastic' in kwargs and kwargs['elastic']:
            config['elastic'] = kwargs['elastic']
        
        # Extract gradient clipping if available
        if 'grad_clip' in kwargs and kwargs['grad_clip']:
            config['grad_clip'] = kwargs['grad_clip']
