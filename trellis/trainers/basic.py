...
class BasicTrainer(Trainer):
    ...
    def __str__(self):
        ...
        lines.append(f'  - Keeping Checkpoints: {self.keep_ckpt}')
        return '\n'.join(lines)
    ...
    def save(self):
        ...
        if os.path.exists(os.path.join(self.output_dir, 'ckpts')):
            old_ckpts = sorted(os.listdir(os.path.join(self.output_dir, 'ckpts')))
            # categorize old checkpoints by model name
            old_ckpt_names = set()
            for ckpt in old_ckpts:
                if ckpt.endswith('.pt'):
                    name = ckpt.split('_')[:-1]  # Remove the last part (e.g., step number)
                    name = '_'.join(name)
                    old_ckpt_names.add(name)
            # for each set of old checkpoints, remove the oldest one if it the checkpints in thath set the keep_ckpt limit
            for name in old_ckpt_names:
                # get all checkpoints for this model
                model_ckpts = [ckpt for ckpt in old_ckpts if ckpt.startswith(f'{name}_step') and ckpt.endswith('.pt')]
                if len(model_ckpts) > self.keep_ckpt:
                    # sort by step number
                    model_ckpts.sort(key=lambda x: int(x.split('_step')[-1].split('.')[0]))
                    # remove the oldest ones until we reach the keep_ckpt limit
                    while len(model_ckpts) > self.keep_ckpt and len(model_ckpts) > 0:
                        print(f'\nRemoving old checkpoint: {model_ckpts[0]}')
                        os.remove(os.path.join(self.output_dir, 'ckpts', model_ckpts[0]))
                        model_ckpts.pop(0)
            ...
            if name in finetune_ckpt:
                ...
                if ckpt_path.endswith('.safetensors'):
                    ...
                    try:
                        ...
                        if self.is_master:
                            print(f'  Loaded safetensors file: {ckpt_path}')
                    except ImportError:
                ...
                # Handle shape mismatches and missing keys
                for k, v in model_ckpt.items():
                    if k not in model_state_dict:
                        if self.is_master:
                            print(f'Warning: {k} not found in model state dict, skipped.')
                        continue
                ...
                # Check for missing keys in the pretrained checkpoint
                missing_keys = set(model_state_dict.keys()) - set(model_ckpt.keys())
                if missing_keys and self.is_master:
                    print(f'Warning: Missing keys in pretrained checkpoint: {missing_keys}')
                    print('These will be randomly initialized.')
                
                # Fill missing keys with current model weights (random initialization)
