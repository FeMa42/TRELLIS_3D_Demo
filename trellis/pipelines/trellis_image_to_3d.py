...
class TrellisImageTo3DPipeline(Pipeline):
    ...
    def _get_execution_device(self) -> torch.device:
        """Get the appropriate device for tensor operations, considering CPU offloading."""
    ...
    def sample_sparse_structure(
        ...
        noise = torch.randn(num_samples, flow_model.in_channels, reso, reso, reso).to(self._get_execution_device())
    ...
    def sample_slat(
        ...
        noise = sp.SparseTensor(
            feats=torch.randn(coords.shape[0], flow_model.in_channels).to(self._get_execution_device()),
            coords=coords,
        )
    ...
    def enable_model_cpu_offload(self, execution_device: Optional[Union[str, torch.device]] = None) -> None:
        ...
        1. Image encoding (image_cond_model)
        2. Sparse structure generation and decoding (sparse_structure_flow_model, sparse_structure_decoder)  
        3. Structured latent generation (slat_flow_model)
        4. Final decoding (slat_decoder_mesh/gs/rf)
