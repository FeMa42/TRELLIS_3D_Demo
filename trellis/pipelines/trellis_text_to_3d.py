...
class TrellisTextTo3DPipeline(Pipeline):
    ...
    def to(self, device: torch.device) -> None:
        """
        Override base class to also move text conditioning model.
        """
        super().to(device)
        if hasattr(self, 'text_cond_model') and self.text_cond_model:
            self.text_cond_model['model'] = self.text_cond_model['model'].to(device)
    ...
    def sample_sparse_structure(
        ...
        coords = torch.argwhere(decoder(z_s)>0)[:, [0, 2, 3, 4]].to(torch.int32).to(self.device)
    ...
    def enable_model_cpu_offload(self, execution_device: Optional[Union[str, torch.device]] = None) -> None:
        """
        Enable stage-aware CPU offloading optimized for text-to-3D pipeline.
        
        The pipeline has 5 main stages:
        1. Text encoding (text_cond_model) 
        2. Sparse structure generation (sparse_structure_flow_model)
        3. Sparse structure decoding (sparse_structure_decoder)
        4. Structured latent generation (slat_flow_model)
        5. Final decoding (slat_decoder_mesh/gs/rf)
        
        Args:
            execution_device: Device for model execution (default: current device or cuda:0)
        """
        super().enable_model_cpu_offload(execution_device)
        print("🎯 Stage-aware offloading enabled for TrellisTextTo3D pipeline")
    ...
    def run(
        ...
        if self._cpu_offload_enabled and self._offload_manager is not None:
            return self._run_with_cpu_offload(prompt, num_samples, sparse_structure_sampler_params, slat_sampler_params, formats)
        else:
    ...
    def _run_with_cpu_offload(
        ...
        # Stage 1: Text encoding - Note: text_cond_model is handled specially
        # It's stored in self.text_cond_model dict, not self.models
        if hasattr(self, 'text_cond_model') and self.text_cond_model:
            # Move text model to GPU temporarily
            text_model = self.text_cond_model['model']
            original_device = text_model.device
            text_model.to(self._offload_manager.execution_device)
            cond = self.get_cond([prompt])
            text_model.to("cpu")  # Move back to CPU
            torch.cuda.empty_cache()
        else:
            cond = self.get_cond([prompt])
        
        # Stage 2: Sparse structure generation  
        with self._offload_manager.stage_context("sparse_structure", ["sparse_structure_flow_model"]):
            coords = self.sample_sparse_structure(cond, num_samples, sparse_structure_sampler_params)
    ...
    def run_variant(
        ...
        coords = torch.cat([
            torch.arange(num_samples).repeat_interleave(coords.shape[0], 0)[:, None].to(torch.int32).to(self.device),
