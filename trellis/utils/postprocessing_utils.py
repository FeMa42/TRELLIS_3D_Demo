...
def bake_texture(
    ...
    brightness_multiplier: float = 1.5,  # Add brightness parameter
    verbose: bool = False,
    ...
    Args:
        ...
        verbose (bool): Whether to print progress.
    """
    # Device management for CPU offloading
    device = execution_device if use_cpu_offload else "cuda"
    
    vertices = torch.tensor(vertices).to(device)
    faces = torch.tensor(faces.astype(np.int32)).to(device)
    uvs = torch.tensor(uvs).to(device)
    
    # Load observations in chunks if using CPU offloading
    if use_cpu_offload:
        # Keep observations on CPU initially, move to GPU in chunks
        observations = [torch.tensor(obs / 255.0).float() for obs in observations]
        masks = [torch.tensor(m>0).bool() for m in masks]
        views = [utils3d.torch.extrinsics_to_view(torch.tensor(extr)) for extr in extrinsics]
        projections = [utils3d.torch.intrinsics_to_perspective(torch.tensor(intr), near, far) for intr in intrinsics]
    else:
    ...
    if mode == 'fast':
        ...
        if use_cpu_offload:
            print(f"🔄 CPU offload mode: Processing {len(observations)} observations in chunks of {chunk_size}")
            # Process observations in chunks to reduce memory usage
            for i in tqdm(range(0, len(observations), chunk_size), disable=not verbose, desc='Texture baking (fast, CPU offload)'):
                chunk_end = min(i + chunk_size, len(observations))
                chunk_obs = [obs.to(device) for obs in observations[i:chunk_end]]
                chunk_masks = [mask.to(device) for mask in masks[i:chunk_end]]
                chunk_views = [view.to(device) for view in views[i:chunk_end]]
                chunk_proj = [proj.to(device) for proj in projections[i:chunk_end]]
                
                for observation, mask, view, projection in zip(chunk_obs, chunk_masks, chunk_views, chunk_proj):
                    with torch.no_grad():
                        ...
                        mask = rast['mask'][0].detach().bool() & mask
        ...
        texture = np.clip(texture.reshape(texture_size, texture_size, 3).cpu().numpy() * 255 * brightness_multiplier, 0, 255).astype(np.uint8)
        ...
        texture = np.clip(texture[0].flip(0).detach().cpu().numpy() * 255 * brightness_multiplier, 0, 255).astype(np.uint8)
...
def to_glb(
    ...
    texture_array = np.clip(texture_array.astype(float) * brightness, 0, 255).astype(np.uint8)
...
def to_glb_new(
    ...
    New Args:
        remove_floating (bool): Whether to remove floating mesh components.
        min_component_ratio (float): Minimum ratio of faces a component must have to be kept.
        min_component_faces (int): Minimum absolute number of faces to keep a component.
        use_voxel_cleanup (bool): Whether to use voxel-based cleanup (more aggressive).
        voxel_resolution (int): Resolution for voxel-based cleanup.
    """
    vertices = mesh.vertices.cpu().numpy()
    faces = mesh.faces.cpu().numpy()
    ...
    if remove_floating:
        if verbose:
            print("Removing floating mesh components...")
        vertices, faces = remove_floating_components(
            vertices, faces,
    ...
    # Optional: More aggressive voxel-based cleanup
    if use_voxel_cleanup:
        if verbose:
            print("Performing voxel-based cleanup...")
        vertices, faces = remove_small_holes_and_islands(
            vertices, faces,
            voxel_resolution=voxel_resolution,
            min_island_ratio=min_component_ratio,
            verbose=verbose
        )
    ...
    extrinsics = [extrinsics[i].cpu().numpy() for i in range(len(extrinsics))]
    intrinsics = [intrinsics[i].cpu().numpy() for i in range(len(intrinsics))]
    # convert vertices to float32 for texture baking
...
def remove_floating_components(
    ...
    """
    Remove floating mesh components that are disconnected from the main mesh.
    ...
    Returns:
        vertices, faces: Cleaned mesh with floating components removed.
    """
    # Create trimesh object for connected components analysis
    ...
    if len(components) <= 1:
        if verbose:
            print("No floating components found")
        return vertices, faces
    
    # Sort components by number of faces (largest first)
    components = sorted(components, key=lambda x: len(x.faces), reverse=True)
    
    # Calculate thresholds
    total_faces = len(faces)
    face_threshold = max(min_component_faces, int(total_faces * min_component_ratio))
    
    # Keep components that meet the threshold
    kept_components = []
    removed_count = 0
    
    for i, component in enumerate(components):
        if len(component.faces) >= face_threshold:
            kept_components.append(component)
            if verbose:
                print(f"Keeping component {i}: {len(component.faces)} faces")
        else:
            removed_count += 1
            if verbose:
                print(f"Removing component {i}: {len(component.faces)} faces (below threshold {face_threshold})")
    
    if verbose:
        print(f"Removed {removed_count} floating components")
    
    # Merge kept components
    if len(kept_components) == 0:
        # Keep the largest component if all are below threshold
    ...
    if len(kept_components) == 1:
        ...
        # Merge multiple large components
        cleaned_mesh = trimesh.util.concatenate(kept_components)
    
    return cleaned_mesh.vertices, cleaned_mesh.faces

def remove_small_holes_and_islands(
    vertices: np.array,
    faces: np.array,
    voxel_resolution: int = 256,
    min_island_ratio: float = 0.01,
    verbose: bool = False
):
    """
    Use voxel-based approach to remove small holes and floating islands.
    This is particularly effective for 3D printing preparation.
    
    Args:
        vertices (np.array): Vertices of the mesh.
        faces (np.array): Faces of the mesh.
        voxel_resolution (int): Resolution for voxelization.
        min_island_ratio (float): Minimum ratio of voxels for an island to be kept.
        verbose (bool): Whether to print progress.
    """
    mesh = trimesh.Trimesh(vertices=vertices, faces=faces, process=False)
    
    # Voxelize the mesh
    voxels = mesh.voxelized(pitch=mesh.bounding_box.extents.max() / voxel_resolution)
    ...
    threshold_voxels = int(total_voxels * min_island_ratio)
    ...
    for label, size in component_sizes:
        if size >= threshold_voxels:
            ...
            if verbose:
                print(f"Keeping voxel component {label}: {size} voxels")
        else:
            if verbose:
                print(f"Removing voxel component {label}: {size} voxels")
    
    # Create cleaned voxel grid
    cleaned_grid = np.zeros_like(voxel_grid)
    for label in keep_labels:
        cleaned_grid[labeled_array == label] = True
    
    # Convert back to mesh
    from skimage import measure
    verts, faces, normals, values = measure.marching_cubes(
        cleaned_grid.astype(float), level=0.5
    )
    
    # Transform back to original coordinate system
    verts = verts * (mesh.bounding_box.extents.max() / voxel_resolution)
    verts += mesh.bounding_box.bounds[0]
...
def to_glb_simple(
    ...
    fill_holes: bool = True,
    fill_holes_max_size: float = 0.04,
    color: tuple = (200, 200, 200),  # Light gray default RGB
    remove_floating: bool = True,
    min_component_ratio: float = 0.05,
    min_component_faces: int = 100,
    use_voxel_cleanup: bool = False,
    ...
    """
    Convert a mesh to a GLB file with a simple solid color, bypassing texture rendering.
    
    This function avoids the 66TB memory allocation bug by not calling render_multiview,
    making it suitable for systems with limited VRAM or when texture rendering fails.
    
    Args:
        mesh (MeshExtractResult): Extracted mesh from TRELLIS.
        simplify (float): Ratio of faces to keep after simplification (0-1).
        fill_holes (bool): Whether to fill holes in the mesh.
        fill_holes_max_size (float): Maximum area of a hole to fill.
        color (tuple): RGB color for the mesh (0-255 per channel).
        remove_floating (bool): Whether to remove floating mesh components.
        min_component_ratio (float): Minimum ratio of faces a component must have to be kept.
        min_component_faces (int): Minimum absolute number of faces to keep a component.
        use_voxel_cleanup (bool): Whether to use voxel-based cleanup (more aggressive).
    ...
    Returns:
        trimesh.Trimesh: The mesh with solid color material ready for GLB export.
    """
    if verbose:
        print("🎨 Creating simple GLB without texture rendering (avoids 66TB bug)")
        print(f"   Using solid color: RGB{color}")
    ...
    # Mesh postprocessing (same as regular GLB export)
    vertices, faces = postprocess_mesh(
        vertices, faces,
        simplify=simplify > 0,
        simplify_ratio=simplify,
    ...
    if verbose:
        print(f"📊 Processed mesh: {vertices.shape[0]} vertices, {faces.shape[0]} faces")
    
    # Rotate mesh (from z-up to y-up) for standard GLB orientation
    vertices = vertices @ np.array([[1, 0, 0], [0, 0, -1], [0, 1, 0]])
    
    # Create vertex colors for the mesh (solid color for all vertices)
    # Each vertex gets the same RGB color plus alpha=255
    vertex_colors = np.ones((len(vertices), 4), dtype=np.uint8)
    vertex_colors[:, 0] = color[0]  # R
    vertex_colors[:, 1] = color[1]  # G
    vertex_colors[:, 2] = color[2]  # B
    vertex_colors[:, 3] = 255        # A (fully opaque)
    ...
    if not mesh.is_watertight:
        mesh.fill_holes()
        if verbose:
            print("🔧 Filled remaining holes for watertight mesh")
    
    if verbose:
        print("✅ Simple GLB mesh created successfully (no texture rendering needed)")
        print(f"   Final mesh: {len(mesh.vertices)} vertices, {len(mesh.faces)} faces")
        print("   Memory usage: Minimal (no texture baking required)")
    
    return mesh


def to_glb_with_cpu_offload(
    app_rep: Union[Strivec, Gaussian],
    mesh: MeshExtractResult,
    simplify: float = 0.95,
    fill_holes: bool = True,
    ...
    chunk_size: int = 5,
    execution_device: str = "cuda:0",
    # Floating component removal parameters
    remove_floating: bool = True,
    min_component_ratio: float = 0.05,
    min_component_faces: int = 100,
    use_voxel_cleanup: bool = False,
    ...
    """
    CPU offloading-enabled version of to_glb_new for memory-efficient texture rendering.
    
    This function is optimized for systems with limited VRAM by using CPU offloading
    and chunked processing during texture baking operations.
    
    Args:
        app_rep: Appearance representation (Strivec or Gaussian)
        mesh: Extracted mesh result
        texture_size: Size of the texture (use smaller values like 512 for lower memory usage)
        use_cpu_offload: Enable CPU offloading for texture operations
        chunk_size: Number of observations to process at once (smaller = less memory)
        execution_device: GPU device to use for computation
        **kwargs: Other parameters passed to to_glb_new
    
    Returns:
        trimesh.Trimesh: The textured mesh ready for GLB export
    """
    if use_cpu_offload:
        print(f"🚀 Starting GLB conversion with CPU offloading (device: {execution_device})")
        print(f"📊 Settings: texture_size={texture_size}, chunk_size={chunk_size}")
    
    # We need to modify the to_glb_new calls to pass CPU offloading parameters
    # For now, let's monkey-patch the bake_texture function temporarily
    import functools
    
    original_bake_texture = bake_texture
    
    def offload_bake_texture(*args, **kwargs):
        if use_cpu_offload:
            kwargs.update({
    ...
    setattr(current_module, 'bake_texture', offload_bake_texture)
    ...
    try:
        ...
        result = to_glb_new(
            ...
            verbose=verbose,
            remove_floating=remove_floating,
            min_component_ratio=min_component_ratio,
            min_component_faces=min_component_faces,
            use_voxel_cleanup=use_voxel_cleanup,
            voxel_resolution=voxel_resolution,
        )
        
        if use_cpu_offload:
            print("✅ GLB conversion completed with CPU offloading")
            
        return result
        
    finally:
        # Restore the original function
        setattr(current_module, 'bake_texture', original_bake_texture)