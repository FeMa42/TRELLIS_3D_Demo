...
def combine_and_remesh() -> None:
    ...
    if combined_object and combined_object.type == 'MESH':
        ...
        remesh_modifier.octree_depth = 8
...
def calculate_origin_bbox() -> dict:
    """Calculates the original bounding box dimensions of the scene before normalization.
    
    This function uses the scene_bbox() function to get the min and max coordinates,
    then calculates the width, height, and length of the bounding box, as well as the
    origin (center point).
    
    Returns:
        dict: A dictionary containing the origin, width, height, and length of the bounding box.
    """
    bbox_min, bbox_max = scene_bbox()
    
    # Calculate dimensions
    dimensions = tuple(max_val - min_val for min_val, max_val in zip(bbox_min, bbox_max))
...
def main(arg):
    ...
    if arg.save_glb_mesh:
        unhide_all_objects()
        combine_meshes()
        convert_to_meshes()
        bpy.ops.export_scene.gltf(filepath=os.path.join(arg.output_folder, 'mesh.glb'))
        origin_bounding_box = calculate_origin_bbox()
        with open(os.path.join(arg.output_folder, 'bounding_box.json'), 'w') as f:
            json.dump(origin_bounding_box, f, indent=4)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_mesh', action='store_true', help='Save the mesh as a .ply file. This will remesh the mesh.')
    parser.add_argument('--save_glb_mesh', action='store_true', help='Save the mesh as a .glb file.')
